...
const ViewHistory = () => {
    ...
    useEffect(() => {
        fetch(`https://wildfire-backend-4muo.onrender.com/viewhistory?uid=${uid}`)// fetch(`http://localhost:5000/viewhistory?uid=${uid}`)
        .then(res => res.json())
    ...
    const handleDelete = async (p: Prediction) => {
        ...
        const response = await fetch('https://wildfire-backend-4muo.onrender.com/viewhistory', { // await fetch('http://localhost:5000/viewhistory', {
            method: 'DELETE',
            body: formData
        });
